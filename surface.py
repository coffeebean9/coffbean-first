...
class Surface(ttk.Frame):
	...
	def __init__(self, win):
		...
		ttk.Label(frame_left, text = '选择车牌照片：', font = ('Arial', 20)).pack(anchor = "nw", padx = 30)
		...
		ttk.Label(frame_right1, text='进行车牌分割：', font = ('Arial', 20)).grid(column = 0, row = 0, sticky = tk.W)
		
		self.roi_ctl = ttk.Label(frame_right1, font = ('Arial', 20))
		self.roi_ctl.grid(column = 0, row = 1)

		ttk.Label(frame_right1, text='识别车牌结果：', font = ('Arial', 20)).grid(column = 0, row = 10, sticky = tk.SW)
		...
		from_folder_ctl = ttk.Button(frame_right2, text="从电脑里选择一张照片", width = 30, command = self.from_folder)
